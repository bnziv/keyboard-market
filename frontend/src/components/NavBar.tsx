...
export default function NavBar({ className }: NavBarProps) {
    ...
    pathname === '/'
      ...
      : pathname.startsWith('/listings')
        ...
        : pathname.startsWith('/create-listing')
          ? 'create'
          : pathname.startsWith('/group-buys')
            ? 'groupbuys'
  ...
  return (
    <header
      className={cn(
        'sticky top-0 z-50 relative flex items-center gap-4 px-6 h-14 border-b bg-km-bg border-km-line',
        className,
      )}
    >
      {/* Brand */}
      ...
      <form
        ...
        className="hidden md:flex flex-1 max-w-sm ml-4"
      >
        <div
      ...
      <nav className="ml-auto flex items-center gap-1">
        ...
        {isAuthenticated ? (
          <DropdownMenu modal={false}>
            ...
            <DropdownMenuContent
              align="end"
              className="bg-km-surface border-km-line text-km-ink"
            >
              <DropdownMenuItem asChild>
                <Link
                  to={`/profile/${user?.username}`}
                  className="flex items-center gap-2 cursor-pointer text-km-ink"
                >
          ...
          >
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md border bg-km-surface-2 border-km-line text-km-ink-mute font-km-mono">
              ...
              <input
                ...
                onChange={(e) => setSearchValue(e.target.value)}
                placeholder="search keycaps, switches, PCBs…"
                className="flex-1 bg-transparent outline-none text-km-ink-dim font-km-mono text-base"
              />
            </div>
          </form>
          {[
